...
def index_summary_file(bucket_name, prefix, output_csv_path):
    s3 = boto3.client('s3', config=Config(signature_version=UNSIGNED), verify=False)
    paginator = s3.get_paginator('list_objects_v2')
    ...
    with open(output_csv_path, write_mode, newline='') as f:
        if header:
            f.write("job_id,filename,start,end,node_count\n")
                ...
                for obj in page['Contents']:
                    ...
                    if key.endswith('-summary.csv'):
                        ...
                        try:
                            ...
                            f.write(f"{jobid},{key},{st_time},{ed_time},{node_count}\n")
                            processed_job_ids.add(jobid)
                            print(f"Processed and wrote: {key}")
                        except Exception as e:
                            print(f"Error processing {key}: {e}")
                            continue
    print(f"Indexing complete. Data saved to {output_csv_path}")
...
if not os.path.exists(fyle):
    ...
    index_summary_file(bucket_name, prefix, fyle)
else: 
